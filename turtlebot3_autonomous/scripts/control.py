...
OBSTACLE_DETECTION_THRESHOLD = 0.4
...
class RobotControl:
   ...
   #Goes to the desired position
    ...
    def rotateToAngle(self, destination_angle):
        print "Rotate to Angle: %f" % destination_angle
...
def request_pos_at_rate(frequency):
    ...
    while not rospy.is_shutdown() and not exit:
        try:
            (trans, rot) = tfListener.lookupTransform('odom', 'base_footprint', rospy.Time(0))
            current_x = trans[0]
            current_y = trans[1]
...
if __name__ == "__main__":
    ...
    print receivedInitPos
    print receivedNewMap
    print receivedNewCostMap
